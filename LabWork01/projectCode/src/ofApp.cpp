...
void ofApp::draw(){
  ...
  /*  for(int i = 1; i < (ofGetWidth() - 1); ++i) {
    ...
    for (int x = 0; x < 5; x+=1) {
        for (int i = 0; i < ofGetWidth()-1; i += 5) {
            ofSetColor(127 + 127 * sin(i * 0.01 + time + x), 127 + 127 * sin(i * 0.011 + time), 127 + 127 * sin(i * 0.012 + time));
            ofDrawCircle(i + 50*sin(i*0.01+x+time), 5+50*sin(waveform[i]) * x, 20 + sin(waveform[i]));
        }
    }
...
void ofApp::audioOut(ofSoundBuffer& output){
    ...
    for (int i = 0; i < output.getNumFrames(); ++i){
        ...
        output[i * outChannels] = myOsc1.coswave(myFreq+myOsc2.sinewave(440)*100) * 0.5* myOsc2.sinewave(myOsc2.sinewave(myOsc2.noise()*50)) ;
